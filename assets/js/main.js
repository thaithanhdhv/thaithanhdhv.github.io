(function ($) {
  ...
  jQuery(document).ready(function () {
    ...
    $("#contactForm").on("submit", function (event) {
      ...
      const mailTitle = `New message from ${name} - ${subject}`;
      const mailBody = `Name: ${name}%0D%0AEmail: ${email}%0D%0AMessage: ${message}`;
      window.open(
        `mailto:thaithanhdhv.vn@gmail.com?subject=${mailTitle}&body=${mailBody}`
      );
    });
  });
})(jQuery);
